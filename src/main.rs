...
fn main() {
    ...
    let p = match provider::by_id(provider_id) {
        Some(p) => p,
        None => {
            println!("{}", format::render_unavailable(provider_id.display_name()));
            return;
        }
    };
    ...
    match p.refresh() {
        Ok(snapshot) => {
            let _ = cache::save(&path, &snapshot);
            println!("{}", format::render(Some(&snapshot)));
        }
        Err(_) => println!("{}", format::render_unavailable(p.display_name())),
    }
}
